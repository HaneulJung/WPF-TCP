...
namespace TCPPrac.Handlers
{
    public class ClientHandler : ChatEventBase
    {
        ...
        public async Task HandleClientAsync()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        InitialData = hub;
                        Debug.WriteLine("클라이언트 연결 이벤트 발생");
                        Connected?.Invoke(this, new ChatEventArgs(this, hub));
                    }
                    else
                    {
                        Debug.WriteLine("클라이언트 데이터 수신 이벤트 발생");
            ...
            {
                Debug.WriteLine($"클라이언트 요청 처리 중 오류 발생: {ex.ToString()}");
            }
            finally
            {
                _client.Close();
        ...
        public void Send(ChatHub hub)
        {
            ...
            {
                Debug.WriteLine($"클라이언트로 메세지 전송 중 오류 발생: {ex.ToString()}");
            }
        }
    }
}
